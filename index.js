...
app.post('/api',require('./deepinfra.js'));
app.use(rpg);
...
app.get('/nsfw',require('./nsfw.js'))
app.get('/otakudesu',require('./otakkudesu/homepage.js'))
app.get('/tes',test)
app.get('/si-barr-suka-hentai', require('./hentai.js'))
app.get('/tiktok', require('./tiktok.js'))
app.post('/tes',test)
...
app.get('/fakeshop', (req, res) => {
  const searchTerm = req.query.search || 'Apa saja yang murah';
  const productThumbnail = req.query.thumbnail || 'https://storage.googleapis.com/a1aa/image/99dea118-5e42-475c-0c90-299bde8bed9c.jpg';
  const productJudul = req.query.judul || 'Produk Contoh';
  const productHarga = req.query.harga || 'Rp 0,00';
     ...
     <body class="bg-white font-sans">
      <header class="flex items-center justify-between px-4 py-2">
       <div class="flex items-center space-x-2">
        <i class="fas fa-flask text-gray-600 text-2xl">
        </i>
       </div>
       <div class="flex justify-center flex-grow">
        <img alt="Google logo with blue, red, yellow, green letters" class="h-10" src="https://upload.wikimedia.org/wikipedia/commons/2/2f/Google_2015_logo.svg"/>
       </div>
       <div>
        <img alt="User avatar of a man with black hair holding a cup" class="rounded-full w-10 h-10 object-cover" height="40" src="https://nirkyy.koyeb.app/favicon.ico" width="40"/>
       </div>
      </header>
      <form aria-label="Search form" class="mx-4 mt-4 flex items-center bg-white rounded-full shadow-md px-4 py-2" role="search">
       <button class="text-gray-600 mr-3" type="submit">
        <i class="fas fa-search text-lg">
        </i>
       </button>
       <input aria-label="Search" class="flex-grow text-gray-800 text-base focus:outline-none" placeholder="Apa saja yang murah" type="search" value="${searchTerm}"/>
       <button aria-label="Clear search" class="text-gray-600 ml-3 focus:outline-none" type="reset">
        <i class="fas fa-times text-lg">
        </i>
       </button>
       <div class="border-l border-gray-300 h-6 mx-3">
       </div>
       <button aria-label="Voice search" class="text-gray-600" type="button">
        <i class="fas fa-microphone text-lg">
        </i>
       </button>
      </form>
      <nav aria-label="Search categories" class="mt-4 mx-4 flex space-x-6 border-b border-gray-200 pb-2 text-sm font-semibold text-gray-600">
       ...
       <a class="text-black border-b-2 border-black pb-1" href="#">
        Shopping
       </a>
       <a class="text-gray-600 hover:text-gray-900" href="#">
        Berita
      ...
      <section class="mt-4 mx-4 flex space-x-3 overflow-x-auto scrollbar-hide pb-3">
       <button class="flex items-center space-x-2 border border-blue-300 bg-blue-100 text-blue-800 rounded-full px-4 py-2 text-sm whitespace-nowrap" type="button">
       ...
       <button class="flex items-center space-x-2 border border-blue-300 bg-blue-100 text-blue-800 rounded-full px-4 py-2 text-sm whitespace-nowrap" type="button">
        <img alt="Small round image representing Buku category" class="rounded-full w-6 h-6 object-cover" height="24" src="https://storage.googleapis.com/a1aa/image/486f4bea-dfab-4192-55d2-e8b505ff1533.jpg" width="24"/>
        <span class="font-semibold">
         Buku
        </span>
       </button>
       <button class="flex items-center space-x-2 border border-blue-300 bg-blue-100 text-blue-800 rounded-full px-4 py-2 text-sm whitespace-nowrap" type="button">
        <img alt="Small round image representing Jam category" class="rounded-full w-6 h-6 object-cover" height="24" src="https://storage.googleapis.com/a1aa/image/81f5f88e-ec13-4419-b743-3e0eea3b9776.jpg" width="24"/>
        <span class="font-semibold">
         Jam Ta
        </span>
       </button>
      </section>
      <section class="mt-4 mx-4 space-y-4">
       <div class="flex space-x-2">
        <article aria-label="Product yang dikustomisasi" class="bg-gray-100 rounded-xl flex-shrink-0 w-44 p-3">
         <img alt="Product thumbnail" class="rounded-lg mb-2 w-full object-cover" height="120" src="${productThumbnail}" width="176"/>
         ...
         <div class="flex items-center space-x-1 mb-1">
          <i class="fab fa-shopify text-orange-600 text-lg">
          </i>
        ...
        <article aria-label="Product SASA Penyedap Rasa 500g" class="bg-gray-100 rounded-xl flex-shrink-0 w-44 p-3">
         <img alt="Product image showing SASA Penyedap Rasa 500g seasoning powder bag" class="rounded-lg mb-2 w-full object-cover" height="120" src="https://i.pinimg.com/736x/fe/aa/9e/feaa9e55dde35f2cc19fb0d15c123a80.jpg" width="176"/>
         <h3 class="font-semibold text-gray-900 text-base mb-1">
          Jual cewek Tepos
         </h3>
         <p class="font-bold text-gray-900 text-base mb-1">
         ...
         <div class="flex items-center space-x-1 mb-1">
          <img alt="Blibli store icon" class="w-5 h-5 object-contain" height="20" src="https://nirkyy.koyeb.app/favicon.ico" width="20"/>
          <span class="text-gray-600 text-sm">
           Nirkyy Store
          </span>
         </div>
...
app.get('/fakeshop-cuy', async (req, res) => {
  const { search, thumbnail, judul, harga } = req.query;

  if (!search || !judul || !harga || !thumbnail) {
    return res.status(400).send('Parameter search, judul, harga, dan thumbnail diperlukan');
  }

  const targetUrl = `https://express-vercel-ytdl.vercel.app/fakeshop?search=${encodeURIComponent(search)}&judul=${encodeURIComponent(judul)}&harga=${encodeURIComponent(harga)}&thumbnail=${encodeURIComponent(thumbnail)}`;
